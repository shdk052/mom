...
def send_email(subject, body, receiver_email):
    ...
    # *** קריאת משתני סביבה של Gmail ***
    EMAIL_USER = os.environ.get('EMAIL_USER')        # המייל שלך
    EMAIL_PASSWORD = os.environ.get('EMAIL_PASSWORD') # סיסמת האפליקציה של Gmail
    SMTP_SERVER = os.environ.get('SMTP_SERVER')      # smtp.gmail.com
    SMTP_PORT = int(os.environ.get('SMTP_PORT', 587))
    SENDER_EMAIL = EMAIL_USER

    if not all([EMAIL_USER, EMAIL_PASSWORD, SMTP_SERVER, SENDER_EMAIL]):
        print("שגיאה קריטית: משתני סביבה של Gmail חסרים.", file=sys.stderr)
        return False
    ...
    try:
        # *** החיבור משתמש בפרטי Gmail, עם Timeout של 60 שניות ***
        # DigitalOcean צפוי לא לחסום את הפורט הזה.
        ...
        print("שגיאת אימות SMTP: סיסמת אפליקציה שגויה או לא פעילה. ודא שהיא תקינה.", file=sys.stderr)
        return False
    except TimeoutError:
        print("שגיאת Timeout: עדיין נחסם על ידי הרשת.", file=sys.stderr)
        return False
    except Exception as e:
        print(f"שגיאה כללית בשליחת מייל: {e}", file=sys.stderr)
        return False
